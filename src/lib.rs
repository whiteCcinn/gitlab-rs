...
mod tests {
    ...
    // use crate::gitlab::A;
    use crate::gitlab::{Client, ListsProjectsResponse};
    ...
    fn test_derive() -> Result<(), Box<dyn std::error::Error>> {
        println!("{:?}", ListGroupsAccessRequestRequest::new("456").get_endpoint());
        Ok(())
    }
//
//     #[test]
...
//         client.set_base_url("http://gitlab.ccinn.com/".to_string()).unwrap();
//
//         let response = client.request(ListGroupsAccessRequestRequest::new(1));
//
//         if response.status().is_success() {
//             let json = response.text().unwrap();
//
