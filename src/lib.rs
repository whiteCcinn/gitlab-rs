...
mod tests {
    ...
    // use crate::gitlab::A;
    use crate::gitlab::{Client, EndPoint, ListsProjectsResponse};
    use crate::group_resources::access_request::{ListGroupsAccessRequestRequest, RequestGroupsAccessRequestRequest, ApproveGroupsAccessRequestRequest};
    use crate::project_resources::access_request::{ListProjectsAccessRequestRequest, RequestProjectsAccessRequestRequest};
    use crate::gitlab;
    use crate::common_resources::payload::{ErrorMessage};
    use crate::common_resources::access_request::{GroupsAccessRequestResponse, ProjectsAccessRequestResponse, AccessLevel, ApproveAccessRequestResponse};
    ...
    fn list_projects() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let json = client.request(gitlab::ListsProjectsRequest::new()).text().unwrap();
    ...
    fn request_group_access_request() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let response = client.request(RequestGroupsAccessRequestRequest::new(3));

        if response.status().is_success() {
            let json = response.text().unwrap();

            println!("{}", json);
            let rs: GroupsAccessRequestResponse = serde_json::from_str(json.as_str()).unwrap();
            println!("{:#?}", rs);
        } else if response.status().is_client_error() {
            let json = response.text().unwrap();
